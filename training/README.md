...
**Pipeline L3 automatisé (local ou CI)** : après `pip install -r requirements.txt`, depuis `training/` exécuter `bash scripts/run_l3_pipeline.sh` (variables optionnelles `AEGIS_L3_EXAMPLES`, `AEGIS_L3_MAX_STEPS`, `AEGIS_L3_MODEL_NAME`, `AEGIS_MODEL_PRODUCT_NAME`, `AEGIS_MODEL_PRODUCT_VERSION`). **Défauts script** : **24 000** exemples synthétiques + **1800** steps (meilleure couverture des tests ONNX qu’avant ; smoke rapide : `AEGIS_L3_EXAMPLES=4000 AEGIS_L3_MAX_STEPS=400`). Le modèle commercial est enregistré sous **ZOKA-SENTINEL** (défaut) avec version dans `zoka_sentinel_manifest.json` et métadonnées ONNX. Sur GitHub : workflow **Helm & NER L3 pipeline** — push `main`/`master` : **120 000** exemples × **2800** `max_steps` (timeout job **180 min**) → artefact `aegis-ner-l3-onnx.tgz` ; **workflow_dispatch** : défauts formulaire **80 000** exemples / **2200** steps (modifiables). Le chart Helm est packagé dans le même workflow (`aegis-helm-chart`) et joint aux **Releases** pour les tags `v*`.
...
| `python -m pytest training/tests/test_l3_sensitive_letter_onnx.py training/tests/test_l3_expert_corpus_onnx.py -q` | Après `run_l3_pipeline.sh` : ONNX INT8 vs lettre FR + corpus expert. Les **marqueurs texte** : `AEGIS_ONNX_MIN_MARKER_PERCENT` (défaut **75**, aligné sur le smoke Helm ; ex. **90** / **95** pour une barre plus haute). **`AEGIS_ONNX_STRICT_MARKERS=1`** impose **100 %**. Chiffres / IBAN / téléphone : strict. |
