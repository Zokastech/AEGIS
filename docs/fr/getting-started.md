...
cd aegis
cp .env.example .env
# Éditer `.env` (depuis `.env.example`) : mots de passe forts ; pour le NER **ZOKA-SENTINEL** en local, voir `ZOKA_SENTINEL_BUNDLE_URL` ou `ZOKA_SENTINEL_FETCH_LATEST` + `ZOKA_SENTINEL_REPO` dans `.env.example`.
```
