...
- [`train_ner_colab.ipynb`](train_ner_colab.ipynb) — même pipeline pour **Google Colab** : clone du dépôt, GPU + `--fp16`, fusion JSONL optionnelle, **inférence rapide**, **`evaluate.py` + baseline Presidio** (F2, rapport HTML), export ONNX, copie optionnelle vers **Google Drive**.
